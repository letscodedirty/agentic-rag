...
## Day 8 — 통합 테스트셋 + 3시스템 평가
- 테스트셋 생성: 단일 파일에 유형 혼합(단일/연쇄/비교/목록/모호), 라벨=정답
  청크 집합, 모호 문항은 intended_query 속성 포함 → 검수 → 확정
- 평가 하네스 확장: DB 경로 주입 + 오라클 루프(SPEC §8) → 3시스템 평가 →
  유형별 비교표 + 발동·오발동 부가 지표 + 정성 대조 몇 건
- **완료 기준: 3시스템 × 유형별 Hit·MRR 비교표 + git tag v2-arch**
