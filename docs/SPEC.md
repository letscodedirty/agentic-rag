...
Day 9 개선 후보(Day 8 본평가 판독): (1) 분류 토큰 매칭 동점 시 최단 우선 →
수록 문서 수 우선 재검토 — "2019년 한국 영화"가 2건짜리 잡분류 '한국의 영화'에
동점 최단으로 밀려 '2019년 영화'(217건)를 놓침(Q32·33 실패). (2) Planner 반례
보강: "특정 영화 한 편의 출연진·제작진을 묻는 질문은 list가 아니라 single" —
명료화 재구성 질문("…출연한 배우들은 누구인가요?")이 list로 분류돼 필모 키
'기생충'으로 list_miss(Q40 체인 결함). (3) 오라클 규칙 4(애매하면 null) 위반
1건 — Q49에서 intended(코미디)와 다른 선택지를 오선택. 하네스 한계로 기록.
