...
## Day 8 — 통합 테스트셋 + 평가
1. [Code] "통합 테스트셋 생성(유형 혼합, 라벨=청크 집합, 모호 문항
   intended_query 포함). 검수 CSV 출력 후 대기."
2. [나] 검수(파트너와) → 통과 → [Code] "하네스 확장(DB 주입+오라클 루프,
   오라클 프롬프트는 승인 대기) → 3시스템 평가 → 비교표+부가 지표 보고.
   완료 기준 확인 후 commit, git tag v2-arch."
